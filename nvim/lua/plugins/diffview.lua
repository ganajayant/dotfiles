		...
		-- buffer so LSP / treesitter / gitsigns attach during review.
		{
			...
			function()
				...
				if base == "" or base == "not found" then
					vim.notify("diffview: could not detect base branch (main/master/develop)", vim.log.levels.WARN)
					return
				end
		...
		-- LuaJIT's arg limit).
		{
			...
			function()
				require("fzf-lua").git_branches({
					...
					preview = "git log --graph --pretty=oneline --abbrev-commit " .. "--color -n 20 {1}",
					actions = {
